...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    private NodeRB put(NodeRB x, Key key, Value value){ // x is the root of the Red-Black subtree
        ...
        //tree balancing
        if (isRed(x.right) && !isRed(x.left)) x = rotateLeft(x); // check if right-leaning red link in subtree, if so, rotate root to the left.
        if (isRed(x.left) && isRed(x.left.left)) x = rotateRight(x); // check if the prior statement caused a double left-leaning (4-node), if so, rotate root to the right.
        if (isRed(x.right) && isRed(x.left)) flipColors(x);// check if a 4-node caused by the prior statement is ready to split into a 2-node, if so, split.
    ...
    private int getHeight(NodeRB x, Key key ){
        if (x == null) return -1;
        int c = key.compareTo(x.key);
        if (c > 0) return 1 + getHeight(x.right,key);
        if (c < 0) return 1 + getHeight(x.left,key);
        else
            return 1;
    }
    ...
    private boolean check(NodeRB x){
        ...
        boolean RLR = true; //Red Left-leaning links in a Row
        boolean blackBalanced = false;
    ...
    private boolean checkOrder(NodeRB x){ // if the put worked then, the only cases of leafs are 3-nodes and 2-nodes (black balanced)
        ...
        if (is2Leaf(x)) return checkOrder(x.left) && checkRRl(x.right);

        int c1 = x.key.compareTo(x.left.key);
        int c2 = x.key.compareTo(x.right.key);
        if (c1 > 0 && c2 < 0)
